...
def ensure_dataset_exists(datasets, dsname, lrecl=93):
    ...
    try:
        ...
        # Hint: options = DatasetOption(dsorg="PS", recfm="FB", lrecl=lrecl, blksize=7161)
        pass
...
def handle_manipulate():
    ...
    if not profile or not input_ds or not output_ds:
        return jsonify({"error": "Missing required fields (credentials or datasets)"}), 400
    ...
    try:
        ...
        # TODO: Step 1 - Ensure input dataset exists (optional for this task, but good practice)
        # TODO: Step 2 - Ensure output dataset exists with LRECL 82 (since SSN is removed)
...
def handle_download_dataset():
    """
    TODO: Implement a route to download a dataset and return its raw content.
    """
    data = request.json
    input_ds = data.get('input_dataset')
    profile = get_profile_from_request(data)
    ...
    try:
        datasets = Datasets(profile)
        # TODO: Step 1 - Download the dataset using datasets.perform_download
        # TODO: Step 2 - Read the file content
        # TODO: Step 3 - Return the content in the JSON response
        return jsonify({
            "status": "success",
            "content": "TODO: Return the actual content here"
        })
    except Exception as e:
